...
from sending.forms import (
    BlockMailingForm,
    BlockMessageForm,
    BlockMessageRecipientForm,
    MailingForm,
    MessageForm,
    ...
    def get_queryset(self):
        ...
        if not queryset:
            ...
            if self.request.user.is_superuser or is_manager:
                ...
                )  # Иначе показываем только собственные сообщения
...
class MailingListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not queryset:
            ...
            if self.request.user.is_superuser or is_manager:
                ...
                queryset = (
                    super().get_queryset().filter(owner=self.request.user)
                )  # Иначе показываем только собственные рассылки
                cache.set(key, queryset, timeout=60)
        logger.info(f"{self.get_queryset.__qualname__}: Успешно")
    ...
    def get_context_data(self, **kwargs):
        ...
        for mailing in mailings:
            ...
            data.append({"mailing": mailing, "total_attempts_count": total_attempts_count})
        context["data"] = data
        context["mailings"] = mailings
    ...
    def get_queryset(self):
        ...
        if not queryset:
            ...
            if self.request.user.is_superuser or is_manager:
                ...
                queryset = (
                    super().get_queryset().filter(owner=self.request.user)
                )  # Иначе показываем только собственных получателей
                cache.set(key, queryset, timeout=60)
        logger.info(f"{self.get_queryset.__qualname__}: Успешно")
